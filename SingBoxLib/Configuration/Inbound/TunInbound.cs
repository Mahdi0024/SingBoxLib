namespace SingBoxLib.Configuration.Inbound;

/// <summary>
/// Represents a Tun inbound configuration.
/// </summary>
public sealed class TunInbound : InboundConfig
{
    /// <summary>
    /// Initializes a new instance of the <see cref="TunInbound"/> class.
    /// </summary>
    /// <param name="tag">The optional inbound tag.</param>
    public TunInbound(string? tag = null)
    {
        Type = "tun";
        Tag = tag;
    }

    /// <summary>
    /// Virtual device name, automatically selected if empty.
    /// </summary>
    [JsonPropertyName("interface_name")]
    public string? InterfaceName { get; set; }

    /// <summary>
    /// IPv4 and IPv6 prefix for the tun interface.
    /// </summary>
    [JsonPropertyName("address")]
    public List<string>? Address { get; set; }

    /// <summary>
    /// The maximum transmission unit for the tun interface.
    /// </summary>
    [JsonPropertyName("mtu")]
    public int? Mtu { get; set; }

    /// <summary>
    /// Set the default route to the Tun.
    /// </summary>
    [JsonPropertyName("auto_route")]
    public bool? AutoRoute { get; set; }

    /// <summary>
    /// Linux iproute2 table index generated by auto_route.
    /// Default value: 2022.
    /// </summary>
    [JsonPropertyName("iproute2_table_index")]
    public int? IpRoute2TableIndex { get; set; }

    /// <summary>
    /// Linux iproute2 rule start index generated by auto_route.
    /// Default value: 9000.
    /// </summary>
    [JsonPropertyName("iproute2_rule_index")]
    public int? IpRoute2RuleIndex { get; set; }

    /// <summary>
    /// Automatically configure iptables/nftables to redirect connections.
    /// Only supported on Linux with <see cref="AutoRoute"/> enabled.
    /// </summary>
    [JsonPropertyName("auto_redirect")]
    public bool? AutoRedirect { get; set; }

    /// <summary>
    /// Connection input mark used by route[_exclude]_address_set with auto_redirect.
    /// Default value: 0x2023
    /// </summary>
    [JsonPropertyName("auto_redirect_input_mark")]
    public string? AutoRedirectInputMark { get; set; }

    /// <summary>
    /// Used with route_exclude_address_set when <see cref="AutoRoute"/> is enabled.
    /// Default value: 0x2024
    /// </summary>
    [JsonPropertyName("auto_redirect_output_mark")]
    public string? AutoRedirectOutputMark { get; set; }

    /// <summary>
    /// Enables strict routing rules to prevent IP address leaks and DNS hijacking on different operating systems.
    /// When true, ensures that all network traffic is routed through a VPN tunnel, mitigating potential privacy risks.
    /// </summary>
    [JsonPropertyName("strict_route")]
    public bool? StrictRoute { get; set; }

    /// <summary>
    /// Enforce strict routing rules when <see cref="AutoRoute"/> is enabled.
    /// </summary>
    [JsonPropertyName("route_address")]
    public List<string>? RouteAddress { get; set; }

    /// <summary>
    /// Add the destination IP CIDR rules in the specified rule-sets to the firewall. Unmatched traffic will bypass the sing-box routes.
    /// </summary>
    [JsonPropertyName("route_address_set")]
    public List<string>? RouteAddressSet { get; set; }

    /// <summary>
    /// Add the destination IP CIDR rules in the specified rule-sets to the firewall.Matched traffic will bypass the sing-box routes.
    /// </summary>
    [JsonPropertyName("route_exclude_address_set")]
    public List<string>? RouteExcludeAddressSet { get; set; }

    /// <summary>
    /// Enable endpoint-independent NAT.
    /// Performance may degrade slightly, so it is not recommended to enable on when it is not needed.
    /// <b>This item is only available on the gvisor stack, other stacks are endpoint-independent NAT by default.</b>
    /// </summary>
    [JsonPropertyName("endpoint_independent_nat")]
    public bool? EndpointIndependentNat { get; set; }

    /// <summary>
    /// TCP/IP stack. see <see href="http://sing-box.sagernet.org/configuration/inbound/tun/#stack">Stack</see>.
    /// Defaults to the mixed stack if the gVisor build tag is enabled, otherwise defaults to the system stack.
    /// </summary>
    [JsonPropertyName("stack")]
    public string? Stack { get; set; }

    /// <summary>
    /// Limit interfaces in route. Not limited by default.
    /// Interface rules are only supported on Linux and require <see cref="AutoRoute"/>.
    /// Conflict with <see cref="ExcludeInterface"/>.
    /// </summary>
    [JsonPropertyName("include_interface")]
    public List<string>? IncludeInterface { get; set; }

    /// <summary>
    /// Exclude interfaces in route.
    /// Conflict with <see cref="IncludeInterface"/>.
    /// </summary>
    [JsonPropertyName("exclude_interface")]
    public List<string>? ExcludeInterface { get; set; }

    /// <summary>
    /// Limit users in route. Not limited by default.
    /// <b>UID rules are only supported on Linux and require <see cref="AutoRoute"/>.</b>
    /// </summary>
    [JsonPropertyName("include_uid")]
    public List<int>? IncludeUid { get; set; }

    /// <summary>
    /// Exclude users in route.
    /// </summary>
    [JsonPropertyName("exclude_uid")]
    public List<int>? ExcludeUid { get; set; }

    /// <summary>
    /// Limit android users in route.
    /// <b>Android user and package rules are only supported on Android and require <see cref="AutoRoute"/>.</b>
    /// </summary>
    [JsonPropertyName("include_android_user")]
    public List<int>? IncludeAndroidUser { get; set; }

    /// <summary>
    /// Limit android packages in route.
    /// </summary>
    [JsonPropertyName("include_package")]
    public List<string>? IncludePackage { get; set; }

    /// <summary>
    /// Limit android packages in route.
    /// </summary>
    [JsonPropertyName("exclude_package")]
    public List<string>? ExcludePackage { get; set; }

    /// <summary>
    /// Exclude custom routes when <see cref="AutoRoute"/> is enabled.
    /// </summary>
    [JsonPropertyName("route_exclude_address")]
    public List<string>? RouteExcludeAddress { get; set; }

    /// <summary>
    /// Limit users in route, but in range.
    /// </summary>
    [JsonPropertyName("include_uid_range")]
    public List<string>? IncludeUidRange { get; set; }

    /// <summary>
    /// Exclude users in route, but in range.
    /// </summary>
    [JsonPropertyName("exclude_uid_range")]
    public List<string>? ExcludeUidRange { get; set; }

    /// <summary>
    /// How DNS is handled on the TUN interface.
    /// </summary>
    [JsonPropertyName("dns_mode")]
    public string? DnsMode { get; set; }

    /// <summary>
    /// List of DNS server addresses used by dns_mode.
    /// </summary>
    [JsonPropertyName("dns_address")]
    public List<string>? DnsAddress { get; set; }

    /// <summary>
    /// Connection reset mark used by auto_redirect pre-matching.
    /// </summary>
    [JsonPropertyName("auto_redirect_reset_mark")]
    public string? AutoRedirectResetMark { get; set; }

    /// <summary>
    /// NFQueue number used by auto_redirect pre-matching.
    /// </summary>
    [JsonPropertyName("auto_redirect_nfqueue")]
    public int? AutoRedirectNfqueue { get; set; }

    /// <summary>
    /// Linux iproute2 fallback rule index generated by auto_redirect.
    /// </summary>
    [JsonPropertyName("auto_redirect_iproute2_fallback_rule_index")]
    public int? AutoRedirectIproute2FallbackRuleIndex { get; set; }

    /// <summary>
    /// Exclude MPTCP connections from transparent proxy.
    /// </summary>
    [JsonPropertyName("exclude_mptcp")]
    public bool? ExcludeMptcp { get; set; }

    /// <summary>
    /// Loopback addresses.
    /// </summary>
    [JsonPropertyName("loopback_address")]
    public List<string>? LoopbackAddress { get; set; }

    /// <summary>
    /// Limit MAC addresses in route.
    /// </summary>
    [JsonPropertyName("include_mac_address")]
    public List<string>? IncludeMacAddress { get; set; }

    /// <summary>
    /// Exclude MAC addresses in route.
    /// </summary>
    [JsonPropertyName("exclude_mac_address")]
    public List<string>? ExcludeMacAddress { get; set; }

    /// <summary>
    /// Platform-specific settings, provided by client applications.
    /// </summary>
    [JsonPropertyName("platform")]
    public TunPlatformSettings? Platform { get; set; }
}

/// <summary>
/// Represents platform-specific settings for the TUN interface.
/// </summary>
public sealed class TunPlatformSettings
{
    /// <summary>
    /// Gets or sets the HTTP proxy settings.
    /// </summary>
    [JsonPropertyName("http_proxy")]
    public TunPlatformHttpServer? HttpProxy { get; set; }

    /// <summary>
    /// Represents the HTTP server settings for the TUN platform.
    /// </summary>
    public sealed class TunPlatformHttpServer
    {
        /// <summary>
        /// Gets or sets whether the HTTP proxy server is enabled.
        /// </summary>
        [JsonPropertyName("enabled")]
        public bool? Enabled { get; set; }

        /// <summary>
        /// Gets or sets the HTTP proxy server address.
        /// </summary>
        [JsonPropertyName("server")]
        public string? Server { get; set; }

        /// <summary>
        /// Gets or sets the HTTP proxy server port.
        /// </summary>
        [JsonPropertyName("server_port")]
        public int? ServerPort { get; set; }

        /// <summary>
        /// Gets or sets the domains to match for the proxy.
        /// </summary>
        [JsonPropertyName("match_domain")]
        public List<string>? MatchDomain { get; set; }

        /// <summary>
        /// Gets or sets the domains to bypass the proxy.
        /// </summary>
        [JsonPropertyName("bypass_domain")]
        public List<string>? BypassDomain { get; set; }
    }
}

/// <summary>
/// Constants representing the network stacks supported by the TUN interface.
/// </summary>
public static class TunStacks
{
    /// <summary>
    /// Use the system TCP/IP stack.
    /// </summary>
    public const string System = "system";

    /// <summary>
    /// Use the gVisor user-space TCP/IP stack.
    /// </summary>
    public const string gVisor = "gvisor";

    /// <summary>
    /// Use a mixed TCP/IP stack.
    /// </summary>
    public const string Mixed = "mixed";
}